  ...
  async execute(message, args, botVer) {
    ...
    if (!searchTerm) {
      const SearchFailed = new Discord.MessageEmbed()
        ...
        .setDescription(
          'Your search term is misssing, please insert a valid search tearm.',
        )
        .setFooter('Error 1')
        .setThumbnail(
      ...
      console.log(
        `${message.author.tag} searched on tenor ${searchTerm} but failed.`,
      );
    } else {
      const url = `https://g.tenor.com/v1/search?q=${searchTerm}&key=${TENOR_KEY}&limit=8`;
      const response = await fetch(url);
      const json = await response.json();
      const index = Math.floor(Math.random() * json.results.length);
      message.channel.send(json.results[index].url);
      console.log(
        `${message.author.tag} searched on tenor ${searchTerm}.`,
      );
    }
  },
};
